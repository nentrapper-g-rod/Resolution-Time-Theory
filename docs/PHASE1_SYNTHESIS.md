...
**Date:** 30 July 2026 (revised after checking the calculations)
...
| 1.4 Bayesian / score route | **Demoted.** Real Poisson score = (k − λ) ∇log I has mean zero. My first simulation hard-coded ∇log I (circular) and used a coefficient that produces I^{1.2}. Does not add an independent non-circular derivation. | Low / withdrawn as independent claim |
...
## What the Poisson score actually says (why I demoted 1.4)

For λ(x) = α I(x) and k ~ Poisson(λ τ):

∂_x log p(k|x) = (k − λ) · (I'/I)

The expectation over k is exactly zero. There is no net drift that climbs the intensity landscape. My earlier claim that “the score contains ∇log I and therefore the estimator locks” conflated the fluctuation term with a mean drift and was implemented by hard-coding the desired drift. I corrected that; the script now demonstrates the mean-score-zero fact and the counting result.
...
3. Quantitative visibility kernels and a concrete gated experiment (Phase 2).

Supporting scripts: `simulations/05`–`09` (08 revised).  
